...
BOOST_AUTO_TEST_CASE(proof_evaluation_worker_retries_local_chain_state_failure)
{
    ...
    const CScript payout = GetScriptForDestination(WitnessUnknown{
        QUANTUM_MIGRATION_WITNESS_VERSION,
        std::vector<unsigned char>(QUANTUM_MIGRATION_PROGRAM_SIZE, 0x5b)});
    uint64_t nonce{92000};
    const CTransactionRef claim =
        MakeCurrentTipIneligibleExactRecoveryTestQQP2Claim(
            chainman, COutPoint{anchor_tx->GetHash(), 0},
            anchor_tx->vout.at(0).nValue, /*fee=*/1000,
            anchor_tx->vout.at(0).scriptPubKey, payout, nonce);
    AddRecoveryTestClaim(*wallet, claim, tip->nHeight,
                         tip->GetBlockHash(), tip->nHeight,
                         tip->GetBlockHash());
    {
...
BOOST_AUTO_TEST_CASE(wallet_tip_hash_and_height_must_both_match)
{
    ...
    for (const auto& [processed_height, processed_hash] :
        ...
        for (unsigned int wait = 0; wait < 300; ++wait) {
...
BOOST_AUTO_TEST_CASE(proof_evaluation_bulk_wallet_maintenance_is_bounded)
{
    RecoveryShadowScheduleGuard schedule{/*whitelist_height=*/99,
                                         /*reward_start_height=*/100,
                                         /*gold_rush_blocks=*/1000};
    ...
    const CScript payout = GetScriptForDestination(WitnessUnknown{
        QUANTUM_MIGRATION_WITNESS_VERSION,
        std::vector<unsigned char>(QUANTUM_MIGRATION_PROGRAM_SIZE, 0x58)});

    auto repair_wallet = CreateSyncedWallet(
        *m_node.chain,
        WITH_LOCK(Assert(m_node.chainman)->GetMutex(),
                  return m_node.chainman->ActiveChain()),
        coinbaseKey);
    uint64_t nonce{80000};
    for (size_t anchor_index : {0U, 1U, 2U}) {
        const CTransactionRef& anchor_tx = m_coinbase_txns.at(anchor_index);
        const COutPoint anchor{anchor_tx->GetHash(), 0};
        const CTransactionRef claim =
            MakeCurrentTipIneligibleExactRecoveryTestQQP2Claim(
                chainman, anchor, anchor_tx->vout.at(0).nValue,
                /*fee=*/1000, CanonicalizeLegacyStakeScript(
                    anchor_tx->vout.at(0).scriptPubKey),
                payout, nonce);
        AddRecoveryTestClaim(
            *repair_wallet, claim, tip->nHeight,
            authored_parent->GetBlockHash(), tip->nHeight,
            tip->GetBlockHash());
    }
    {
        LOCK(repair_wallet->cs_wallet);
        repair_wallet->ClearShadowPowClaimProofEvaluationCacheForTesting();
    }
    ResetShadowPowClaimRecoveryProofEvaluationStatsForTesting();
    ResetShadowArgon2EvaluationCountForTesting();
    repair_wallet->RepairStaleShadowTransactions(/*force=*/true);
    // One bounded inventory pass evaluates each distinct context exactly once.
    // Metadata repair performs no ProcessTransaction proof validation under
    // cs_main, so total Argon2 calls cannot exceed that lock-free count.
    BOOST_CHECK_EQUAL(
        GetShadowPowClaimRecoveryProofEvaluationCountForTesting(), 3U);
    BOOST_CHECK_EQUAL(GetShadowArgon2EvaluationCountForTesting(), 3U);
    BOOST_CHECK_EQUAL(
        GetShadowPowClaimRecoveryInventoryBuildCountForTesting(), 1U);

    // Startup/chainstate force repair can run before mempool.dat republishes
    // a claim that was live at shutdown. That temporary absence must not
    // create a durable first-quarantine timestamp and change the component's
    // restart-stable audit fingerprint. A later periodic pass can record a
    // real continuing absence.
    auto startup_wallet = CreateSyncedWallet(
        *m_node.chain,
        WITH_LOCK(Assert(m_node.chainman)->GetMutex(),
                  return m_node.chainman->ActiveChain()),
        coinbaseKey);
    const CTransactionRef& startup_anchor_tx = m_coinbase_txns.at(6);
    const CTransactionRef startup_claim =
        MakeCurrentTipEligibleExactRecoveryTestQQP2Claim(
            chainman, COutPoint{startup_anchor_tx->GetHash(), 0},
            startup_anchor_tx->vout.at(0).nValue, /*fee=*/1000,
            CanonicalizeLegacyStakeScript(
                startup_anchor_tx->vout.at(0).scriptPubKey),
            payout, nonce);
    AddRecoveryTestClaim(
        *startup_wallet, startup_claim, tip->nHeight,
        authored_parent->GetBlockHash(), tip->nHeight, tip->GetBlockHash());
    {
        LOCK(startup_wallet->cs_wallet);
        CWalletTx& stored =
            startup_wallet->mapWallet.at(startup_claim->GetHash());
        stored.mapValue.erase(SHADOW_POW_QUARANTINE_MARKER_KEY);
        stored.mapValue.erase(
            SHADOW_POW_CLAIM_FIRST_QUARANTINE_HEIGHT_KEY);
        stored.mapValue.erase(SHADOW_POW_CLAIM_FIRST_QUARANTINE_TIP_KEY);
        stored.mapValue.erase(
            SHADOW_POW_CLAIM_BRANCH_QUARANTINE_HEIGHT_KEY);
        stored.mapValue.erase(SHADOW_POW_CLAIM_BRANCH_QUARANTINE_TIP_KEY);
        startup_wallet->ClearShadowPowClaimProofEvaluationCacheForTesting();
    }
    const ShadowPowClaimRecoveryInventory startup_before_force =
        startup_wallet->GetShadowPowClaimRecoveryInventory();
    BOOST_REQUIRE_EQUAL(startup_before_force.components.size(), 1U);
    startup_wallet->RepairStaleShadowTransactions(/*force=*/true);
    {
        LOCK(startup_wallet->cs_wallet);
    ...
    const ShadowPowClaimMiningGate startup_pending_gate =
        startup_wallet->GetShadowPowClaimMiningGate();
    BOOST_CHECK(!startup_pending_gate.HasUnsafeClaims());
    BOOST_CHECK(startup_pending_gate.ShouldRelayExisting() ||
                startup_pending_gate.MayRefreshSameAnchor());

    // Model successful mempool restoration: the existing callback clears the
    // transient marker. With no manufactured first-observation metadata, the
    // complete component fingerprint returns to its exact pre-force value.
    {
        LOCK(startup_wallet->cs_wallet);
        startup_wallet->mapWallet.at(startup_claim->GetHash())
            .mapValue.erase(SHADOW_POW_QUARANTINE_MARKER_KEY);
    }
    const ShadowPowClaimRecoveryInventory startup_after_restoration =
        startup_wallet->GetShadowPowClaimRecoveryInventory();
    BOOST_REQUIRE_EQUAL(startup_after_restoration.components.size(), 1U);
    BOOST_CHECK(startup_after_restoration.components.front().fingerprint ==
                startup_before_force.components.front().fingerprint);

    // If restoration stays absent, another force pass recreates only the
    // transient marker. The first periodic non-force pass then sees that
    // pending candidate and records one real post-startup observation without
    // needing a block or wallet mutation to unstick the family.
    startup_wallet->RepairStaleShadowTransactions(/*force=*/true);
    startup_wallet->RepairStaleShadowTransactions(/*force=*/false);
    {
        LOCK(startup_wallet->cs_wallet);
        const CWalletTx& stored =
            startup_wallet->mapWallet.at(startup_claim->GetHash());
        BOOST_REQUIRE_EQUAL(
            stored.mapValue.at(SHADOW_POW_QUARANTINE_MARKER_KEY), "1");
        BOOST_CHECK(stored.mapValue.count(
                        SHADOW_POW_CLAIM_FIRST_QUARANTINE_HEIGHT_KEY) == 1);
        BOOST_CHECK(stored.mapValue.count(
                        SHADOW_POW_CLAIM_FIRST_QUARANTINE_TIP_KEY) == 1);
    }
    BOOST_CHECK(!startup_wallet->GetShadowPowClaimMiningGate()
                     .HasUnsafeClaims());

    auto resubmit_wallet = CreateSyncedWallet(
        *m_node.chain,
        WITH_LOCK(Assert(m_node.chainman)->GetMutex(),
                  return m_node.chainman->ActiveChain()),
        coinbaseKey);
    for (size_t anchor_index : {3U, 4U, 5U}) {
        const CTransactionRef& anchor_tx = m_coinbase_txns.at(anchor_index);
        const COutPoint anchor{anchor_tx->GetHash(), 0};
        const CTransactionRef claim =
            MakeCurrentTipEligibleExactRecoveryTestQQP2Claim(
                ...
                payout, nonce);
        AddRecoveryTestClaim(
            *resubmit_wallet, claim, tip->nHeight,
            authored_parent->GetBlockHash(), tip->nHeight,
            tip->GetBlockHash());
        LOCK(resubmit_wallet->cs_wallet);
        CWalletTx& stored =
            resubmit_wallet->mapWallet.at(claim->GetHash());
        stored.mapValue["qq_reorg_shadow_resubmit"] = "1";
    }
    resubmit_wallet->SetBroadcastTransactions(/*broadcast=*/true);
    {
        LOCK(resubmit_wallet->cs_wallet);
        resubmit_wallet->ClearShadowPowClaimProofEvaluationCacheForTesting();
    }
    const ShadowPowClaimMiningGate warm_gate =
        resubmit_wallet->GetShadowPowClaimMiningGate();
    BOOST_REQUIRE(warm_gate.ShouldRelayExisting());
    ResetShadowPowClaimRecoveryProofEvaluationStatsForTesting();
    ResetShadowArgon2EvaluationCountForTesting();
    resubmit_wallet->ResubmitWalletTransactions(
        /*relay=*/false, /*force=*/false);
    // The warm inventory is reused, while the scheduler selects at most one
    // exact family for ordinary mempool validation in this tick. N retained
    // shadow claims therefore cannot cause N locked proof evaluations.
    BOOST_CHECK_EQUAL(
        GetShadowPowClaimRecoveryProofEvaluationCountForTesting(), 0U);
    BOOST_CHECK_LE(GetShadowArgon2EvaluationCountForTesting(), 1U);
    ResetShadowPowClaimRecoveryProofEvaluationStatsForTesting();
    ResetShadowArgon2EvaluationCountForTesting();
}

BOOST_AUTO_TEST_CASE(force_repair_retry_exhaustion_is_bounded_and_durable)
{
    RecoveryShadowScheduleGuard schedule{/*whitelist_height=*/99,
                                         /*reward_start_height=*/100,
                                         /*gold_rush_blocks=*/1000};
    auto reset_hook = interfaces::MakeCleanupHandler([] {
        SetShadowPowClaimRepairRetryFailureForTesting(
    ...
    const auto exercise = [&](
                             ...
                             tip->GetBlockHash());
        {
            ...
            // Persist the exact unmarked starting point used by the restart
            // fixture, rather than relying on a live-only mapValue edit.
            BOOST_REQUIRE(WalletBatch(wallet->GetDatabase()).WriteTx(stored));
            wallet->ClearShadowPowClaimProofEvaluationCacheForTesting();
        }

        ResetShadowPowClaimRecoveryProofEvaluationStatsForTesting();
        SetShadowPowClaimRepairRetryFailureForTesting(failure);
        ...
        SetShadowPowClaimRepairRetryFailureForTesting(
            ShadowPowClaimRepairRetryFailureForTesting::NONE);
        BOOST_CHECK_LT(elapsed_ms, 5000);
        BOOST_CHECK_EQUAL(
            GetShadowPowClaimRecoveryInventoryBuildCountForTesting(), 3U);
        {
            LOCK(wallet->cs_wallet);
            const CWalletTx& stored =
                wallet->mapWallet.at(claim->GetHash());
            BOOST_REQUIRE_EQUAL(stored.mapValue.at(
        ...
        const MockableData persisted =
            GetMockableDatabase(*wallet).m_records;
        auto reloaded = std::make_unique<CWallet>(
            m_node.chain.get(), "", CreateMockableWalletDatabase(persisted));
        BOOST_REQUIRE_EQUAL(reloaded->LoadWallet(), DBErrors::LOAD_OK);
        {
            LOCK(reloaded->cs_wallet);
            reloaded->SetLastBlockProcessed(tip->nHeight,
                                            tip->GetBlockHash());
            const CWalletTx& stored =
                reloaded->mapWallet.at(claim->GetHash());
            BOOST_REQUIRE_EQUAL(stored.mapValue.at(
                                    SHADOW_POW_QUARANTINE_MARKER_KEY),
                                "1");
            BOOST_CHECK(stored.mapValue.count(
        ...
        // No new block, generation, or external RPC is needed: the first
        // periodic pass consumes the durable pending marker and records the
        // first real post-startup absence.
        reloaded->RepairStaleShadowTransactions(/*force=*/false);
        {
            LOCK(reloaded->cs_wallet);
            const CWalletTx& stored =
                reloaded->mapWallet.at(claim->GetHash());
        ...
        BOOST_CHECK(!reloaded->GetShadowPowClaimMiningGate()
                         .HasUnsafeClaims());
    };

    exercise(ShadowPowClaimRepairRetryFailureForTesting::SNAPSHOT_DRIFT, 7);
    exercise(
        ShadowPowClaimRepairRetryFailureForTesting::WALLET_LOCK_CONTENTION,
        8);

    // A failed reservation write never leaves a clean-looking unreserved
    // family. The live wallet latches ambiguity; a reload from the proven
    // pre-attempt records has no marker and can retry normally.
    auto failed = CreateSyncedWallet(
        *m_node.chain,
        WITH_LOCK(Assert(m_node.chainman)->GetMutex(),
                  return m_node.chainman->ActiveChain()),
        coinbaseKey);
    const CTransactionRef& failed_anchor_tx = m_coinbase_txns.at(9);
    const CTransactionRef failed_claim =
        MakeCurrentTipEligibleExactRecoveryTestQQP2Claim(
            chainman, COutPoint{failed_anchor_tx->GetHash(), 0},
            failed_anchor_tx->vout.at(0).nValue, /*fee=*/1000,
            CanonicalizeLegacyStakeScript(
                failed_anchor_tx->vout.at(0).scriptPubKey),
            payout, nonce);
    AddRecoveryTestClaim(*failed, failed_claim, tip->nHeight,
                         parent->GetBlockHash(), tip->nHeight,
                         tip->GetBlockHash());
    {
        LOCK(failed->cs_wallet);
        CWalletTx& stored = failed->mapWallet.at(failed_claim->GetHash());
        stored.mapValue.erase(SHADOW_POW_QUARANTINE_MARKER_KEY);
        stored.mapValue.erase(SHADOW_POW_CLAIM_FIRST_QUARANTINE_HEIGHT_KEY);
        stored.mapValue.erase(SHADOW_POW_CLAIM_FIRST_QUARANTINE_TIP_KEY);
        BOOST_REQUIRE(WalletBatch(failed->GetDatabase()).WriteTx(stored));
    }
    const MockableData failed_before =
        GetMockableDatabase(*failed).m_records;
    GetMockableDatabase(*failed).m_fail_begin = true;
    failed->RepairStaleShadowTransactions(/*force=*/true);
    GetMockableDatabase(*failed).m_fail_begin = false;
    {
        LOCK(failed->cs_wallet);
        BOOST_CHECK(failed->IsShadowPowClaimRecoveryDatabaseAmbiguous());
        BOOST_CHECK(failed->mapWallet.at(failed_claim->GetHash())
                        .mapValue.count(
                            SHADOW_POW_QUARANTINE_MARKER_KEY) == 0);
    }
    BOOST_CHECK(failed->GetShadowPowClaimMiningGate().HasUnsafeClaims());
    auto clean_retry = std::make_unique<CWallet>(
        m_node.chain.get(), "", CreateMockableWalletDatabase(failed_before));
    BOOST_REQUIRE_EQUAL(clean_retry->LoadWallet(), DBErrors::LOAD_OK);
    {
        LOCK(clean_retry->cs_wallet);
        clean_retry->SetLastBlockProcessed(tip->nHeight,
                                           tip->GetBlockHash());
    }
    clean_retry->RepairStaleShadowTransactions(/*force=*/true);
    BOOST_CHECK(WITH_LOCK(
        clean_retry->cs_wallet,
        return clean_retry->mapWallet.at(failed_claim->GetHash())
                   .mapValue.count(SHADOW_POW_QUARANTINE_MARKER_KEY) == 1));
}
    ...
    RecoveryQQP2TestingSetup)
{
    ...
    // phase and preserves same-snapshot suppression.
...
BOOST_AUTO_TEST_CASE(resolution_metadata_is_recognized_only_when_complete)
{
    ...
    BOOST_REQUIRE(wallet->AddToWallet(
        ...
        [&](CWalletTx& wtx, bool) {
            ...
            wtx.mapValue[SHADOW_POW_RESOLUTION_CREATED_HEIGHT_KEY] =
                ToString(tip->nHeight);
            wtx.mapValue[SHADOW_POW_RESOLUTION_CREATED_TIME_KEY] = "1";
            // Exact released v30.1.4 draft shape: authorization was
            // canonical, while the later revocation byte did not exist.
            wtx.mapValue[SHADOW_POW_RESOLUTION_RELAY_AUTHORIZED_KEY] = "0";
            return true;
        }));
    ...
    BOOST_CHECK(!managed_node->resolution_relay_authorized);
    BOOST_CHECK(!managed_node->resolution_relay_revoked);
...
BOOST_AUTO_TEST_CASE(managed_resolution_authority_upgrade_matrix_is_exact)
{
    auto wallet = CreateSyncedWallet(
        *m_node.chain,
        WITH_LOCK(Assert(m_node.chainman)->GetMutex(),
    ...
    BOOST_REQUIRE(!generation.IsNull());
    ...
    const CTransactionRef resolution_ref =
        MakeTransactionRef(std::move(resolution));
    BOOST_REQUIRE(wallet->AddToWallet(
        resolution_ref, TxStateInactive{}, [&](CWalletTx& wtx, bool) {
            wtx.mapValue[SHADOW_POW_RESOLUTION_SCHEMA_KEY] =
                SHADOW_POW_RESOLUTION_SCHEMA_VERSION;
            wtx.mapValue[SHADOW_POW_RESOLUTION_ANCHOR_TXID_KEY] =
                anchor.hash.GetHex();
            ...
            wtx.mapValue[SHADOW_POW_RESOLUTION_CREATED_HEIGHT_KEY] =
                ToString(tip->nHeight);
            wtx.mapValue[SHADOW_POW_RESOLUTION_CREATED_TIME_KEY] = "1";
            // Released v30.1.4 wrote this mandatory byte, but had no
            // relay-revoked key.
            wtx.mapValue[SHADOW_POW_RESOLUTION_RELAY_AUTHORIZED_KEY] = "0";
            return true;
        }));

    const auto set_authority = [&](std::optional<std::string> authorized,
                                   std::optional<std::string> revoked) {
        LOCK(wallet->cs_wallet);
        CWalletTx& wtx = wallet->mapWallet.at(resolution_ref->GetHash());
        if (authorized) {
        ...
        if (revoked) {
            ...
            wtx.mapValue.erase(SHADOW_POW_RESOLUTION_RELAY_REVOKED_KEY);
        }
        BOOST_REQUIRE(WalletBatch(wallet->GetDatabase()).WriteTx(wtx));
        wallet->RefreshShadowPowClaimIndexEntryLocked(wtx);
        wallet->MarkShadowPowClaimCandidateStateChangedLocked();
    };
    const auto check_authority = [&](bool valid, bool authorized,
                                     bool revoked) {
        const ShadowPowClaimRecoveryInventory inventory =
            wallet->GetShadowPowClaimRecoveryInventory();
        const auto& component = FindRecoveryComponent(inventory, anchor);
        const auto node = std::find_if(
            component.nodes.begin(), component.nodes.end(),
            [&](const auto& candidate) {
                return candidate.txid == resolution_ref->GetHash();
            });
        BOOST_REQUIRE(node != component.nodes.end());
        if (!valid) {
            BOOST_CHECK(!component.has_managed_resolution);
            BOOST_CHECK(component.has_indeterminate_node);
        ...
        BOOST_CHECK(component.has_managed_resolution);
        BOOST_CHECK(node->resolution_metadata_valid);
        BOOST_CHECK_EQUAL(node->resolution_relay_authorized, authorized);
        BOOST_CHECK_EQUAL(node->resolution_relay_revoked, revoked);
    };

    // Exact released draft and commit encodings: absent revoke normalizes to
    // false, while authorization remains mandatory and canonical.
    check_authority(/*valid=*/true, /*authorized=*/false,
                    /*revoked=*/false);
    set_authority("1", std::nullopt);
    check_authority(/*valid=*/true, /*authorized=*/true,
                    /*revoked=*/false);

    // New revocation encoding is canonical and restriction-only. Conflicting,
    // malformed, or missing authority bytes remain nonauthorizing.
    set_authority("0", "1");
    check_authority(/*valid=*/true, /*authorized=*/false,
                    /*revoked=*/true);
    set_authority("1", "1");
    check_authority(/*valid=*/false, /*authorized=*/false,
                    /*revoked=*/false);
    set_authority("0", "invalid");
    check_authority(/*valid=*/false, /*authorized=*/false,
                    /*revoked=*/false);
    set_authority(std::nullopt, "0");
    check_authority(/*valid=*/false, /*authorized=*/false,
                    /*revoked=*/false);
}

...
| Cancel future local relay of one exact managed resolution | No wallet unlock or anchor release; signed bytes remain auditable and may still confirm | `revokeshadowpowclaimresolution <resolution_txid> true`; a fresh exact-plan commit is the only wallet path that clears the durable tombstone |

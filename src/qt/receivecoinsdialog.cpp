...
namespace {
constexpr int RECEIVE_ADDRESS_TYPE_QUANTUM = -1000;
} // namespace
    ...
    platformStyle(_platformStyle)
{
    ...
    connect(ui->addressType, qOverload<int>(&QComboBox::currentIndexChanged), this, [this] { updateAddressTypeNotice(); });
...
void ReceiveCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        add_address_type(OutputType::LEGACY, tr("Legacy Blackcoin (current chain)"), tr("Creates a normal legacy Blackcoin address for current-chain BLK receiving."));
        if (!model->wallet().privateKeysDisabled()) {
            const auto index = ui->addressType->count();
            ui->addressType->addItem(tr("Quantum-resistant ML-DSA (upgraded wallet)"), RECEIVE_ADDRESS_TYPE_QUANTUM);
            ui->addressType->setItemData(index, tr("Creates a wallet-backed Blackcoin ML-DSA address for migration, Gold Rush rewards, and post-quantum sends."), Qt::ToolTipRole);
        }
...
void ReceiveCoinsDialog::updateAddressTypeNotice()
{
    const bool quantum = ui->addressType->currentData().toInt() == RECEIVE_ADDRESS_TYPE_QUANTUM;
    if (quantum) {
        ui->label_5->setText(tr("<b>Quantum-resistant ML-DSA address.</b> Use this with upgraded Blackcoin wallets for migration, Gold Rush rewards, and post-quantum sends. Do not hand this to a legacy-only sender before they upgrade."));
        ui->label_5->setStyleSheet(QStringLiteral("QLabel { color: #6a4100; background: #fff6d6; padding: 6px; border-radius: 4px; }"));
        ui->receiveButton->setText(tr("Create quantum ML-DSA address"));
    } else {
        ui->label_5->setText(tr("<b>Legacy Blackcoin address.</b> Use this for current-chain BLK receiving from legacy wallets before migration. This is not a quantum-resistant destination."));
        ui->label_5->setStyleSheet(QStringLiteral("QLabel { color: #1f4d2b; background: #edf8ef; padding: 6px; border-radius: 4px; }"));
        ui->receiveButton->setText(tr("Create legacy Blackcoin address"));
    }
}
...
void ReceiveCoinsDialog::on_receiveButton_clicked()
{
    ...
    const int address_type_id = ui->addressType->currentData().toInt();
    QString address;

    if (address_type_id == RECEIVE_ADDRESS_TYPE_QUANTUM) {
        WalletModel::UnlockContext ctx(model->requestUnlock());
        if (!ctx.isValid()) return;

        auto result = model->wallet().createQuantumAddress(label.toStdString());
        if (!result) {
        ...
        address = QString::fromStdString(result->address);
    } else {
        /* Generate new receiving address */
        const OutputType address_type = (OutputType)address_type_id;

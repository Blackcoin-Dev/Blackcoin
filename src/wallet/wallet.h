...
struct ShadowPowClaimStakeReserveInfo {
    ...
    size_t mature_stakeable_legacy_coins{0};
    CAmount mature_stakeable_legacy_weight{0};
    size_t reserved_stake_coins{0};
...
inline constexpr std::string_view StakingTelemetryStateName(
    StakingTelemetryState state) noexcept
{
    switch (state) {
    case StakingTelemetryState::DISABLED: return "disabled";
    case StakingTelemetryState::STARTING: return "starting";
    case StakingTelemetryState::LOCKED: return "locked";
...
/** Stable, internally coherent PoS worker snapshot for RPC/GUI monitoring. */
struct StakingTelemetrySnapshot {
    uint64_t sequence{0};
    StakingTelemetryState state{StakingTelemetryState::DISABLED};
    bool enabled{false};
    ...
    std::string reason{"staking is disabled"};
};
...
enum class ShadowPowClaimSubmitResult {
    SUBMITTED,
    /** Exact bytes were durably retained/quarantined but relay failed. The
     * miner must re-enter the typed family gate before doing any more work. */
    PERSISTED_PENDING,
    INPUT_UNAVAILABLE,
    FAILED,
};
...
/** Build legacy-compatible RPC counters from a recovery snapshot without
 * performing a second proof-classification pass. */
ShadowPowClaimInventory BuildShadowPowClaimCompatibilityInventory(
    const ShadowPowClaimRecoveryInventory& recovery_inventory);
...
private:
     ...
     */
    static bool AttachChainUnpublished(const std::shared_ptr<CWallet>& wallet, interfaces::Chain& chain, const bool rescan_required, bilingual_str& error, std::vector<bilingual_str>& warnings);
...
public:
    ...
    /** Sign using script flags captured from the same cs_main-pinned snapshot
     * as the caller's wallet mutation. This avoids reacquiring cs_main while
     * cs_wallet is held by atomic recovery paths. */
    bool SignTransactionWithScriptVerifyFlags(CMutableTransaction& tx, std::map<int, bilingual_str>& input_errors, unsigned int script_verify_flags) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    /** Sign the tx given the input coins and sighash. */
    bool SignTransaction(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors) const;
    /** Sign with input coins and an already pinned script-policy snapshot. */
    bool SignTransactionWithScriptVerifyFlags(CMutableTransaction& tx, const std::map<COutPoint, Coin>& coins, int sighash, std::map<int, bilingual_str>& input_errors, unsigned int script_verify_flags) const;
    ...
    bool SubmitTxMemoryPoolAndRelay(
        ...
        std::optional<CAmount> max_tx_fee_override = std::nullopt);
    ...
    StakingTelemetrySnapshot m_staking_telemetry
        GUARDED_BY(m_staking_telemetry_mutex);
    ...
    // Serialize that deferred first-use path so multiple configured workers
    // cannot race to create or bind different payout keys.
    Mutex m_pow_payout_mutex;
    ...
    // Worker 0 refreshes the typed, component-aware mining gate and publishes
    // one coherent snapshot for the remaining hashing workers. This avoids N
    // identical proof classifications per second; final submission still
    // performs an authoritative tip-pinned recheck.
    ...
    // wait_for_normal_unlock is reserved for postInitProcess after explicit
    // -powmining=1 consent. Interactive callers must retain the strict default.
    bool SetPowMining(bool enabled, int threads, int cpu_percent, bilingual_str& error,
                      ...
                      bool wait_for_normal_unlock = false);
    void StopPowMining();
    ...
    bool AdvanceShadowPowClaimRelayClockHighWatersForTesting(
        int64_t schema_time, int64_t legacy_time)
    {
        LOCK(cs_wallet);
        return AdvanceShadowPowClaimRelayClockHighWatersDurably(
            schema_time, legacy_time);
    }
    std::pair<int64_t, int64_t>
    GetShadowPowClaimRelayClockHighWatersForTesting() const
    {
        LOCK(cs_wallet);
        return {m_shadow_pow_claim_relay_clock_high_water,
    ...
    GetShadowPowClaimRecoveryUsageFromInventoryLocked(
        ...
        uint32_t rolling_window_seconds) const
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main, cs_wallet);
    ShadowPowClaimRecoveryReview GetShadowPowClaimRecoveryReview(
        const ShadowPowClaimRecoveryRequest& request) const;
    ...
    bool GetShadowPowClaimRefreshCoinLocked(
        const ShadowPowClaimInput& selected,

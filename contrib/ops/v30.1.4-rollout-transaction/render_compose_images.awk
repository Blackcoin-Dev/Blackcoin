...
BEGIN {
    ...
    for (i = 1; i <= n; i++) {
        if (requested[i] !~ /^node([1-9]|[12][0-9]|3[0-2])$/ || (requested[i] in wanted)) exit 64
...
in_services && /^  node([1-9]|[12][0-9]|3[0-2]):[[:space:]]*$/ {
    current = $1
    sub(/:$/, "", current)
...
in_services && /^  [A-Za-z0-9_.-]+:[[:space:]]*$/ && $0 !~ /^  node([1-9]|[12][0-9]|3[0-2]):/ {
    current = ""
}

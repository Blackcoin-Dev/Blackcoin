...
# Read-only verifier for a sealed post-release hotfix-candidate bundle. It does
...
fail()
{
    printf 'hotfix-candidate verification failed: %s\n' "$*" >&2
    exit 1
}
...
prefix="Blackcoin-30.1.4-hotfix-candidate-${source_commit:0:12}"
checksums="$prefix-SHA256SUMS.txt"
readonly source_commit base_manifest base_config prefix checksums
...
printf 'VERIFIED_CANDIDATE_BUNDLE=%s\nSOURCE_COMMIT=%s\nBASE_MANIFEST=%s\nBASE_CONFIG=%s\n' \
    "$BUNDLE" "$source_commit" "$base_manifest" "$base_config"

...
# Pure predicates for the node30 Free-Claim audit and its deliberately blocked
# release interface. The current public RPC cannot enforce an exact fee input,
# a maximum total fee, or an atomic one-shot dispatch/re-pause transaction, so
# this contract contains no pause-marker transition primitive.
...
readonly V3015_NODE30_REQUIRED_SOURCE_SHA='0e62ec0af3daefba30f87382d9b3cc8b00224e62'
readonly V3015_NODE30_REQUIRED_TREE='d460eee11b7c8c6d5fffe6935f2e9a5d58e18aac'
readonly V3015_NODE30_REQUIRED_MERGE='e85668ed26ef75d92e234488cbd85e146f6ffd5a'
readonly V3015_NODE30_REQUIRED_PARENT_MAIN='19baffef25af36e177db2975780e0641b59753aa'
readonly V3015_NODE30_REQUIRED_PARENT_SOURCE='0e62ec0af3daefba30f87382d9b3cc8b00224e62'
readonly V3015_NODE30_REQUIRED_CI_RUN=31710198720
...
v3015_validate_node30_audit_env()
{
    ...
    for name in NODE30_PAUSE_WRAPPER_SHA256 NODE30_PAUSE_MARKER_SHA256 \
        ...
        NODE30_MAINTENANCE_FINALIZATION_RECEIPT_SHA256 NODE30_FLEET_RESULT_SHA256 \
        NODE30_PACKAGING_VERIFIER_RECEIPT_SHA256; do
    ...
    for name in NODE30_PUBLIC_ARTIFACT_AUTHORITY \
        ...
        NODE30_MAINTENANCE_FINALIZATION_RECEIPT NODE30_FLEET_RESULT \
        NODE30_PACKAGING_VERIFIER_RECEIPT; do
        v3015_require_resolved "$name" "${!name:-}" || return
...
{
    v3015_validate_node30_audit_env || return
    # Release has no satisfiable authority schema in this candidate. Keeping
    # this validator hard-false prevents a placeholder or invented receipt from
    # turning an audit package into a marker-removal capability.
    v3015_die 'node30 release_eligible=false: exact-input and atomic one-shot interfaces are unresolved'
    return 1
}

v3015_node30_public_artifact_authority_is_valid()
{
    local file=$1 release_identity_sha image_manifest packaging_sha
    release_identity_sha=$(v3015_sha256_file "$RELEASE_IDENTITY_JSON") || return
    packaging_sha=$(v3015_sha256_file "$NODE30_PACKAGING_VERIFIER_RECEIPT") || return
    [[ "$packaging_sha" == "$NODE30_PACKAGING_VERIFIER_RECEIPT_SHA256" ]] || return
    jq -e --arg source "$V3015_NODE30_REQUIRED_SOURCE_SHA" \
      --arg tree "$V3015_NODE30_REQUIRED_TREE" --arg merge "$V3015_NODE30_REQUIRED_MERGE" \
      ...
      --argjson run "$V3015_NODE30_REQUIRED_CI_RUN" '
        type == "object" and (keys | sort) == (["kind","merge_parents","merge_sha",
          "merge_tree","registry_manifest_sha256","run_attempt","run_id","schema",
          "source_sha","source_tree","status"] | sort) and
        .schema == 1 and .kind == "v30.1.5-packaging-verifier" and .status == "PASS" and
        .source_sha == $source and .source_tree == $tree and .merge_sha == $merge and
        .merge_tree == $tree and .merge_parents == [$parent_main,$parent_source] and
        .registry_manifest_sha256 == $manifest and .run_id == $run and .run_attempt == 1
      ' "$NODE30_PACKAGING_VERIFIER_RECEIPT" >/dev/null || return
    ...
    jq -e --arg source "$SOURCE_SHA" --arg tree "$SOURCE_TREE" \
      ...
      --arg release_identity "$release_identity_sha" --arg packaging "$packaging_sha" \
      --arg required_source "$V3015_NODE30_REQUIRED_SOURCE_SHA" \
      --arg required_tree "$V3015_NODE30_REQUIRED_TREE" \
      --arg merge "$V3015_NODE30_REQUIRED_MERGE" \
      --arg parent_main "$V3015_NODE30_REQUIRED_PARENT_MAIN" \
      --arg parent_source "$V3015_NODE30_REQUIRED_PARENT_SOURCE" \
      --argjson required_run "$V3015_NODE30_REQUIRED_CI_RUN" '
        ...
        def expected_keys: ["artifact","authorized_utc","binary_sha256s",
          ...
          "subversion","merge_sha","merge_tree","merge_parents",
          "merge_signature_verified","merge_signature_verifier","strict_main_head_sha",
          "packaging_verifier_receipt_sha256","packaging_verifier_run_id",
          "packaging_verifier_run_attempt","artifact_provenance_source_tree",
          "artifact_provenance_registry_manifest_sha256"];
        type == "object" and (keys | sort) == (expected_keys | sort) and
        .schema == 1 and .kind == "v30.1.5-public-artifact-authority" and
        .release == "v30.1.5" and .source_sha == $required_source and
        .source_tree == $required_tree and $source == $required_source and $tree == $required_tree and
        .merge_sha == $merge and .merge_tree == $required_tree and
        .merge_parents == [$parent_main,$parent_source] and
        .merge_signature_verified == true and
        .merge_signature_verifier == $fingerprint and .strict_main_head_sha == $merge and
        .packaging_verifier_receipt_sha256 == $packaging and
        .packaging_verifier_run_id == $required_run and .packaging_verifier_run_attempt == 1 and
        .artifact_provenance_source_tree == $required_tree and
        .artifact_provenance_registry_manifest_sha256 == $image_manifest and
        .source_signature_verified == true and
        .source_signing_fingerprint == $fingerprint and
        .core_ci == {run_id:$required_run,head_sha:$required_source,
          conclusion:"success",workflow:$workflow} and $ci_run == $required_run and
        .artifact == {name:$artifact,run_id:$required_run,run_attempt:1} and
        $artifact_run == $required_run and $attempt == 1 and
    ...
    jq -e --arg source "$SOURCE_SHA" --arg tree "$SOURCE_TREE" \
      --arg public "$public_sha" --arg worker "$NODE30_ORIGINAL_WORKER_SHA256" '
        ...
        def expected_keys: ["authenticated_lineaged_claim_requires_distinct_utxo",
          ...
          "ordinary_pow_required","product_test_receipt_available","product_test_receipt_sha256",
          "public_artifact_authority_sha256","pure_foreign_audit_history_ignored",
          "release","release_eligible","interface_blockers",
          "retained_family_preserves_payout_anchor_lineage",
        ...
        .interface_blockers == ["exact_fee_input_not_bindable_before_broadcast",
          ...
          "atomic_repause_and_terminal_receipt_absent"] and
        .worker_fee_cap_enforced == false and
        .worker_queue_result_binds_actual_payout == false and
        .product_test_receipt_available == false and .product_test_receipt_sha256 == null and
        (.reviewed_utc | utc)
      ' "$file" >/dev/null
}
...
{
    local file=$1 release_identity_sha public_sha semantics_sha evidence census authority
    local census_sha authority_sha terminal_probe_sha
    release_identity_sha=$(v3015_sha256_file "$RELEASE_IDENTITY_JSON") || return
    public_sha=$(v3015_sha256_file "$NODE30_PUBLIC_ARTIFACT_AUTHORITY") || return
    semantics_sha=$(v3015_sha256_file "$NODE30_SUCCESSOR_SEMANTICS_RECEIPT") || return
    [[ "$(v3015_sha256_file "$NODE30_FLEET_RESULT")" == "$NODE30_FLEET_RESULT_SHA256" ]] || return
    evidence=$(dirname -- "$NODE30_FLEET_RESULT") || return
    census="$evidence/terminal-fleet-census.json"
    authority="$evidence/rollout-authority.json"
    [[ -f "$census" && ! -L "$census" && -f "$authority" && ! -L "$authority" ]] || return
    census_sha=$(v3015_sha256_file "$census") || return
    authority_sha=$(v3015_sha256_file "$authority") || return
    terminal_probe_sha=$(jq -er '.node30_terminal_probe_sha256' "$census") || return
    v3015_terminal_census_is_valid "$census" "$evidence" "$authority" || return
    v3015_fleet_result_is_valid "$NODE30_FLEET_RESULT" "$census_sha" \
      "$terminal_probe_sha" "$authority" || return
    jq -e --arg source "$SOURCE_SHA" --arg tree "$SOURCE_TREE" \
      --arg image "$CANDIDATE_IMAGE_REF" --arg image_id "$CANDIDATE_IMAGE_ID" \
      ...
      --arg census "$census_sha" --arg rollout_authority "$authority_sha" \
      --arg marker "$NODE30_PAUSE_MARKER_SHA256" \
      --slurpfile fleet_result "$NODE30_FLEET_RESULT" --slurpfile terminal "$census" '
        def utc: type == "string" and
          test("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$");
        def expected_keys: ["candidate_image_id","candidate_image_ref",
          ...
          "release_identity_sha256","rollout_authority_sha256","schema","source_sha","source_tree",
          "terminal_census_sha256","deployment_state",
        ...
        .fleet_result_sha256 == $fleet and .terminal_census_sha256 == $census and
        .rollout_authority_sha256 == $rollout_authority and
        .free_claim_pause_marker_sha256 == $marker and
        .maintenance_transaction_state == "pause_preserved_pending_separate_release" and
        ...
        ($fleet_result|length) == 1 and ($terminal|length) == 1 and
        $fleet_result[0].deployment_state == .deployment_state and
        $fleet_result[0].status == "PAUSE_PRESERVED_PENDING_SEPARATE_RELEASE" and
        $fleet_result[0].node30_free_claim_healthy == true and
        $fleet_result[0].node30_free_claim_paused == true and
        $terminal[0].deployment_state == .deployment_state and
        .fleet_healthy_nodes == ($terminal[0].nodes|length) and
        .fleet_pos_active_nodes == $fleet_result[0].pos_active and
        .regular_pow_active_nodes == $fleet_result[0].regular_pow_operational and
        .fleet_pos_active_nodes == 32 and .regular_pow_active_nodes == 31 and
        $fleet_result[0].pos_active_nodes == [range(1;33)] and
        $fleet_result[0].regular_pow_nodes == ([range(1;30)] + [31,32]) and
        .data_rewind_used == false and
        (.completed_utc | utc)
      ' "$file" >/dev/null
}
...
{
    # No receipt can bridge the candidate's missing exact-input, total-fee, and
    # atomic one-shot/re-pause interfaces. This predicate intentionally has no
    # accepting input.
    return 1
}
...
{
    # Compatibility name remains fail-closed; release authority is unresolved.
    return 1
}
...
v3015_node30_make_safe_audit_receipt()
{
    ...
    work_sha=$(jq -S -c '.work' "$snapshot" | sha256sum | awk '{print $1}') || return
    snapshot_sha=$(v3015_sha256_file "$snapshot") || return
    public_sha=$(v3015_sha256_file "$NODE30_PUBLIC_ARTIFACT_AUTHORITY") || return
    semantics_sha=$(v3015_sha256_file "$NODE30_SUCCESSOR_SEMANTICS_RECEIPT") || return
    ...
    jq -c --arg source "$SOURCE_SHA" --arg image "$CANDIDATE_IMAGE_REF" \
      ...
      --arg audit_tool "${NODE30_AUDIT_TOOL_SHA256:-}" --arg work_sha "$work_sha" \
      --arg snapshot_sha "$snapshot_sha" '
        {schema:1,kind:"v30.1.5-node30-free-claim-release-audit",source_sha:$source,
         audit_tool_sha256:$audit_tool,snapshot_sha256:$snapshot_sha,
         ...
         observed_candidate_fee_outpoint_sha256:$outpoint_sha,
         fee_input_binding_available:false,work_identity_sha256:$work_sha,
         ...
         witness_version:16,release_ready:false,release_eligible:false,action:"audit-only",
         interface_blockers:["exact_fee_input_not_bindable_before_broadcast",
           "max_total_fee_not_enforced_by_rpc","reviewed_one_shot_dispatcher_absent",
...
v3015_node30_audit_receipt_is_valid()
{
    local file=$1 expected_tool=$2
    jq -e --arg source "$SOURCE_SHA" --arg image "$CANDIDATE_IMAGE_REF" \
      --arg tool "$expected_tool" --arg marker "$NODE30_PAUSE_MARKER_SHA256" \
      --arg wrapper "$NODE30_PAUSE_WRAPPER_SHA256" --arg worker "$NODE30_ORIGINAL_WORKER_SHA256" '
        type == "object" and .schema == 1 and
        .kind == "v30.1.5-node30-free-claim-release-audit" and
        .source_sha == $source and .candidate_image_ref == $image and .node == 30 and
        .audit_tool_sha256 == $tool and
        ([.snapshot_sha256,.work_identity_sha256,.wallet_identity_sha256,
          .observed_candidate_fee_outpoint_sha256,.queue_item_sha256,
          .queue_address_sha256,.quantum_payout_script_sha256] |
          all(type == "string" and test("^[0-9a-f]{64}$"))) and
        .pause_marker_sha256 == $marker and .pause_wrapper_sha256 == $wrapper and
        .worker_sha256 == $worker and .ordinary_pow_enabled == false and
        .ordinary_pow_hashrate == 0 and .blocking_wallet_relevant_families == 0 and
        .broadcast_records == 0 and .action == "audit-only" and
        .release_ready == false and .release_eligible == false and
        .fee_input_binding_available == false and .marker_preserved == true and
        .raw_transaction_exposed == false and .payout_address_exposed == false and
        .interface_blockers == ["exact_fee_input_not_bindable_before_broadcast",
          "max_total_fee_not_enforced_by_rpc","reviewed_one_shot_dispatcher_absent",
          "atomic_repause_and_terminal_receipt_absent"]
      ' "$file" >/dev/null
...
v3015_node30_audit_matches_current()
{
    local prior=$1 current=$2
    jq -e -n --argjson prior "$prior" --argjson current "$current" '
      ($prior | del(.observed_epoch,.unlock_remaining_seconds,.snapshot_sha256)) ==
      ($current | del(.observed_epoch,.unlock_remaining_seconds,.snapshot_sha256)) and
      $prior.tip == $current.tip and $prior.height == $current.height and
      $prior.wallet_identity_sha256 == $current.wallet_identity_sha256 and
      $prior.observed_candidate_fee_outpoint_sha256 ==
        $current.observed_candidate_fee_outpoint_sha256 and
      $prior.queue_item_sha256 == $current.queue_item_sha256 and
      $prior.queue_address_sha256 == $current.queue_address_sha256 and
      $prior.quantum_payout_script_sha256 == $current.quantum_payout_script_sha256 and
      $prior.work_identity_sha256 == $current.work_identity_sha256
    ' >/dev/null
}

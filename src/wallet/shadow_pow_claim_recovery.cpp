                                 ...
                                 ManagedResolutionFacts& facts)
{
    ...
    facts.relay_revoked = false;
    const auto relay_authorized = wtx.mapValue.find(
        SHADOW_POW_RESOLUTION_RELAY_AUTHORIZED_KEY);
    const auto relay_revoked = wtx.mapValue.find(
        SHADOW_POW_RESOLUTION_RELAY_REVOKED_KEY);
    const bool relay_revoked_valid =
        relay_revoked == wtx.mapValue.end() ||
    ...
    if (origin == wtx.mapValue.end() ||
        ...
        !ParsePositiveMetadataTime(wtx.mapValue,
                                   ...
                                   facts.created_time) ||
        relay_authorized == wtx.mapValue.end() ||
        !ParseCanonicalMetadataBool(
...
void CWallet::RefreshShadowPowClaimIndexEntryLocked(const CWalletTx& wtx)
{
    ...
    if (wtx.tx->GetTotalSize() >
        ...
        m_shadow_pow_recycled_claim_txids.erase(txid);
        return;
    }
    ...
    if (locally_relevant) {
        for (const CTxIn& input : wtx.tx->vin) {
            if (input.prevout.n != 0 ||
                m_shadow_pow_managed_resolution_txids.count(
    ...
    if (recycled) {
        m_shadow_pow_recycled_claim_txids.insert(txid);
    } else {
        m_shadow_pow_recycled_claim_txids.erase(txid);
    }
}
...
void CWallet::RefreshShadowPowClaimOwnershipIndex() const
{
    ...
    while (classified < SHADOW_POW_CLAIM_OWNERSHIP_SLICE) {
        ...
        if (!has_shadow_proof) {
            ...
            wallet.m_shadow_pow_recycled_claim_txids.erase(txid);
...
void CWallet::RebuildShadowPowClaimIndexesLocked()
{
    ...
    m_shadow_pow_recycled_claim_txids.clear();
    ...
    // DB key order can load a claim before its managed parent. Re-evaluate
    // the compact local set after every record and state is available.
    m_shadow_pow_recycled_claim_txids.clear();
    for (const uint256& txid : m_shadow_pow_claim_txids) {
        const auto it = mapWallet.find(txid);
        if (it != mapWallet.end()) {
            RefreshShadowPowClaimIndexEntryLocked(it->second);
        }
    }
}
    ...
    std::vector<ShadowPowClaimProofEvaluationContext>* all_contexts) const
{
    ...
    for (ShadowPowClaimRecoveryComponent& component : inventory.components) {
        ...
        if (component.anchor_unspent && component.has_indeterminate_node) {
            component.state = ShadowPowClaimRecoveryState::INDETERMINATE;
            component.adoption_graph_safe = false;
        }
    ...
    const ShadowPowClaimMiningGate& deferred_gate)
{
    ...
    if ((!deferred_gate.ShouldRelayExisting() &&
        ...
        tip->nHeight != deferred_gate.active_height ||
...
ShadowPowClaimRecoveryUsage BuildRecoveryUsageLocked(
    ...
    const std::set<uint256>& managed_resolution_txids,
    const std::set<uint256>& recycled_claim_txids)
{
    AssertLockHeld(::cs_main);
    AssertLockHeld(wallet.cs_wallet);
    ...
    for (const uint256& txid :
         managed_resolution_txids) {
        const auto wallet_it = wallet.mapWallet.find(txid);
        if (wallet_it == wallet.mapWallet.end()) continue;
        const CWalletTx& wtx = wallet_it->second;
        ManagedResolutionFacts facts;
    ...
    usage.recycled_outputs = recycled_claim_txids.size();
    return usage;
}
    ...
    const std::set<uint256>& recycled_claim_txids)
{
    ...
    if (request.origin == ShadowPowClaimRecoveryOrigin::AUTOMATIC) {
        ...
        automatic_usage = BuildRecoveryUsageLocked(
            ...
            managed_resolution_txids, recycled_claim_txids);
    }
    ...
    int active_height, int64_t created_time, std::string& error)
{
    ...
    for (uint32_t output = 0; output < committed.tx->vout.size(); ++output) {
        const auto spenders = mapTxSpends.equal_range(
            COutPoint{txid, output});
    ...
    const ShadowPowClaimRecoveryRequest& request) const
{
    ...
    return BuildRecoveryPlanLocked(*this, effective,
                                   automatic_policy
                                       ...
                                       : nullptr,
                                   inventory,
                                   m_shadow_pow_managed_resolution_txids,
                                   m_shadow_pow_recycled_claim_txids);
}
    ...
    const ShadowPowClaimRecoveryRequest& request)
{
    ...
    std::set<uint256> relay_promote_after_success;
    uint64_t post_persist_generation{0};
    {
        ...
        if (!ShadowPowClaimRecoveryInventoryMatchesCurrentLocked(
            ...
            result.error = "tip or wallet state changed while evaluating recovery proof contexts";
            return result;
        }
        ...
        for (size_t i = 0; i < result.plan.actions.size(); ++i) {
            ...
            if (newly_signed) {
                ...
                {
                    // Wallet signing may traverse script managers or invoke
                    // an external provider. Drop both global locks, then hold
                    // cs_wallet alone so key state is coherent without
                    // blocking validation.
                    REVERSE_LOCK(criticalblock2);
                    {
                        ...
                        signed_transaction =
                            ...
                            !IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER) &&
                            SignTransactionWithScriptVerifyFlags(
                                mutable_tx, input_errors,
                                script_verify_flags);
                    }
                }
                ...
                if (!ShadowPowClaimRecoveryInventoryMatchesCurrentLocked(
                        planning_inventory)) {
                    result.stale_plan = true;
                    result.error = "tip or wallet state changed during recovery signing";
                    return result;
                }
            }
            ...
            if (!action.in_mempool) {
                std::optional<MempoolAcceptResult> accept;
                {
                    // Test acceptance can execute expensive script and proof
                    // policy. It holds only validation's required cs_main and
                    // never runs beneath the wallet/recovery snapshot locks.
                    REVERSE_LOCK(criticalblock2);
                    {
                        REVERSE_LOCK(criticalblock1);
                        LOCK(::cs_main);
                        accept.emplace(chain().chainman().ProcessTransaction(
                ...
                if (!ShadowPowClaimRecoveryInventoryMatchesCurrentLocked(
                        planning_inventory)) {
                    result.stale_plan = true;
                    result.error = "tip or wallet state changed during recovery validation";
                    return result;
                }
                if (accept->m_result_type !=
        ...
        {
        // Durable recovery publication is serialized by cs_wallet and the
        // recovery-authority mutex, but never by cs_main. Exact tip drift is
        // checked immediately after the batch; any bytes already committed
        // remain non-broadcast and tip-bound until a fresh plan.
        REVERSE_LOCK(criticalblock2);
        {
        REVERSE_LOCK(criticalblock1);
        LOCK(cs_wallet);
        if (m_shadow_pow_claim_recovery_db_ambiguous ||
            m_locked_coins_db_ambiguous) {
            result.durable_state_ambiguous = true;
            result.error = "recovery database authority became ambiguous before persistence";
            return result;
        }
        if (GetDatabase().nUpdateCounter.load() !=
                planning_inventory.wallet_generation ||
            GetShadowPowClaimCandidateStateFingerprintLocked() !=
                planning_inventory.candidate_state_fingerprint) {
            result.stale_plan = true;
            result.error = "wallet claim state changed before recovery persistence";
            return result;
        }
        if (effective.origin == ShadowPowClaimRecoveryOrigin::AUTOMATIC &&
            (!automatic_policy ||
            ...
            result.error = "automatic recovery authority was revoked before persistence";
            return result;
        }
        for (Prepared& item : prepared) {
            ...
            if (item.newly_signed) {
                ...
                result.durable_state_changed = true;
                if (commit_authority) {
                    relay_promote_after_success.insert(
                        item.transaction->GetHash());
                }
                item.relay_authorized = false;
        ...
        const CBlockIndex* post_persist_tip =
            chain().chainman().ActiveChain().Tip();
        if (!post_persist_tip ||
            post_persist_tip->GetBlockHash() != result.plan.active_tip ||
            post_persist_tip->nHeight != result.plan.active_height ||
            m_last_block_processed != result.plan.active_tip ||
            m_last_block_processed_height != result.plan.active_height) {
            result.stale_plan = true;
            result.error = "active or wallet-processed tip changed during recovery persistence; exact signed bytes remain stored but were not relayed";
            return result;
        }
    }
    ...
    for (size_t relay_index = 0; relay_index < relay_txids.size();
        ...
        if (!tip || tip->GetBlockHash() != result.plan.active_tip ||
            ...
            if (managed && !relay_authorized &&
                relay_promote_after_success.count(txid) == 0) {
        ...
        if (already_in_mempool) {
            if (relay_promote_after_success.count(txid) != 0) {
                LOCK(cs_wallet);
                if (!SetManagedShadowPowResolutionRelayAuthority(
                        txid, /*authorized=*/true,
                ...
                auto& action = result.plan.actions.at(
                    relay_action_indices.at(txid));
        ...
        broadcast_guard.allow_current_call_draft_relay =
            relay_promote_after_success.count(txid) != 0;
        if (effective.origin ==
            ShadowPowClaimRecoveryOrigin::AUTOMATIC) {
        ...
        if (relay_promote_after_success.count(txid) != 0) {
            ...
            if (!SetManagedShadowPowResolutionRelayAuthority(
                ...
                result.durable_state_ambiguous =
                    m_shadow_pow_claim_recovery_db_ambiguous;
                result.error =
                    "signed resolution was relayed, but durable standing "
                    "relay authority could not be recorded: " + result.error;
                return result;
            }
            result.durable_state_changed = true;
            ++result.relay_authority_granted;
            auto& promoted = result.plan.actions.at(
                relay_action_indices.at(txid));
            promoted.relay_authorized = true;
            promoted.relay_revoked = false;
        }
        result.plan.actions.at(relay_action_indices.at(txid)).status =
    ...
    const uint256& expected_component_fingerprint)
{
    ...
    // Adoption is restriction-only provenance publication. Revalidate every
    // wallet fact after dropping the global chain lock, then perform the one
    // durable batch with cs_wallet alone. A concurrent reorg can only make
    // the exact tip-bound metadata stale/fail-closed; it must never be able to
    // hold validation behind database I/O.
    LOCK(cs_wallet);
    if (m_shadow_pow_claim_recovery_db_ambiguous ||
        m_locked_coins_db_ambiguous) {
        result.durable_state_ambiguous = true;
        return fail(
            ShadowPowClaimRecoveryAdoptionStatus::DATABASE_OUTCOME_AMBIGUOUS,
            "recovery database authority became ambiguous before adoption persistence");
    }
    if (GetDatabase().nUpdateCounter.load() != inventory.wallet_generation ||
        ...
        return fail(
            ShadowPowClaimRecoveryAdoptionStatus::STALE_TIP,
            "wallet claim state changed before adoption persistence; review the component again");
    }
    ...
    // instead of rebuilding the classifier (or depending on an incidental
    // expensive-proof cache) while the mutation locks are held.
    ...
    // UNKNOWN provenance is the only adoption-curable indeterminate fact in
    // a graph that passed SafeGraphForAdoption above. Re-derive the two
    // component aggregates that depend on it exactly as a fresh inventory
    // pass will after the durable metadata commit.
